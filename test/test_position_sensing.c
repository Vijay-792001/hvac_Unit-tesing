...
/* Provide extern instance for ADC handle */
ADC_HandleTypeDef hadc1;

/* Internal test support */
void setUp(void)
{
    /* Always reset mock call counters */
...
{
    /* All state zeroed, validity cleared */
    ...
    /* pos should stay unchanged since GetPosition returned 0 */
...
void test_PS_03_Update_sets_valid_when_ADC_poll_OK_highest_stop(void)
{
    PositionSensing_Init();

    HAL_ADC_Start_Expect(&hadc1);
    HAL_ADC_PollForConversion_ExpectAndReturn(&hadc1, 2, HAL_OK);
    /* ADC value in stop range 0 region (4060) */
...
/* ========== PS_08: Map ADC to position 5 region (lowest) ========== */
...
void test_PS_09_Update_sets_invalid_when_ADC_poll_fails(void)
{
    ...
    uint8_t pos = 0xCD;
    TEST_ASSERT_EQUAL_UINT8(0, PositionSensing_GetPosition(&pos));
    TEST_ASSERT_EQUAL_UINT8(0xCD, pos); // unchanged
}

/* ========== PS_10: GetPosition returns last stored mapped value after successful update ========== */
void test_PS_10_GetPosition_returns_stored_after_Update(void)
{
    PositionSensing_Init();
...
void test_PS_11_IsAtTarget_rejects_out_of_range_target(void)
{
    ...
    /* Return 0 if target >= 6U, regardless of value */
    TEST_ASSERT_EQUAL_UINT8(0, PositionSensing_IsAtTarget(6));
    TEST_ASSERT_EQUAL_UINT8(0, PositionSensing_IsAtTarget(22));
}

/* ========== PS_12: IsAtTarget returns 1 when adc inside stop range ========== */
void test_PS_12_IsAtTarget_returns_1_when_adc_in_stop_range(void)
{
    /* Set up so s_adc_value is in "stop" window for target 0: 4055 - 4065 */
    PositionSensing_Init();
    ...
    PositionSensing_Update();
    /* Now s_adc_value should be 4060 */
...
{
    /* Target 0 range: 4055 - 4065; use 4040 */
    PositionSensing_Init();
...
{
    /* For target 3: 3309 - 3329; use 3320 */
    PositionSensing_Init();
